...
static HRESULT CreateSceneAssets() noexcept {
    ...
    hr = g_D3DDevice->CreateDepthStencilState(&dsDesc, &g_SkyboxDepthState);
    if (FAILED(hr)) return hr;
...
static void UpdateModelBuffer(float dt) noexcept {
    ...
    _In_ int nCmdShow)
{
    ...
    HWND hMainWindow = CreateWindowEx(
        0,
        wc.lpszClassName,
        L"Direct3D 11 - Textured Cube with Skybox",
        WS_OVERLAPPEDWINDOW,
