...
ID3D11DepthStencilState* g_DepthStateNoWrite = nullptr;
...
static HRESULT CreateExtraResources() {
    HRESULT hr = S_OK;
    ...
    hr = g_D3DDevice->CreateBuffer(&matDesc, nullptr, &g_MaterialBuffer);
    if (FAILED(hr)) return hr;
    ...
    hr = g_D3DDevice->CreateBlendState(&blendDesc, &g_BlendState);
    if (FAILED(hr)) return hr;
    ...
    hr = g_D3DDevice->CreateDepthStencilState(&dsDesc, &g_DepthStateNoWrite);
    if (FAILED(hr)) return hr;
...
static void DestroyD3DResources() {
    ...
    SafeRelease(g_DepthStateNoWrite);
...
static void UpdateModelBuffer(const XMMATRIX& model) {
...
static void UpdateViewProjBuffer(const XMMATRIX& viewProj) {
    XMFLOAT4X4 vpData;
...
static void UpdateMaterialBuffer(float r, float g, float b, float a) {
    D3D11_MAPPED_SUBRESOURCE mapped;
    HRESULT hr = g_ImmediateContext->Map(g_MaterialBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped);
    if (SUCCEEDED(hr)) {
        MaterialBuffer* pMat = (MaterialBuffer*)mapped.pData;
        pMat->color = XMFLOAT4(r, g, b, a);
        g_ImmediateContext->Unmap(g_MaterialBuffer, 0);
    }
}
...
static void RenderFrame() {
    ...
    g_ImmediateContext->OMSetDepthStencilState(nullptr, 0);

    UpdateViewProjBuffer(viewProj);

    float currentTime = static_cast<float>(GetTickCount64()) / 1000.0f;

    XMMATRIX modelOpaque = XMMatrixRotationY(currentTime * 0.5f);
    UpdateModelBuffer(modelOpaque);
    UpdateMaterialBuffer(1.0f, 1.0f, 1.0f, 1.0f);

    D3D11_DEPTH_STENCIL_DESC dsDescOpaque = {};
    dsDescOpaque.DepthEnable = TRUE;
    dsDescOpaque.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
    dsDescOpaque.DepthFunc = D3D11_COMPARISON_LESS;
    dsDescOpaque.StencilEnable = FALSE;
    ID3D11DepthStencilState* pDSOpaque = nullptr;
    g_D3DDevice->CreateDepthStencilState(&dsDescOpaque, &pDSOpaque);
    g_ImmediateContext->OMSetDepthStencilState(pDSOpaque, 0);

    D3D11_RASTERIZER_DESC rsDescOpaque = {};
    rsDescOpaque.FillMode = D3D11_FILL_SOLID;
    rsDescOpaque.CullMode = D3D11_CULL_BACK;
    rsDescOpaque.FrontCounterClockwise = FALSE;
    ID3D11RasterizerState* pRSCube = nullptr;
    g_D3DDevice->CreateRasterizerState(&rsDescOpaque, &pRSCube);
    ...
    ID3D11Buffer* cbsCube[] = { g_ModelBuffer, g_ViewProjBuffer, g_MaterialBuffer };
    g_ImmediateContext->VSSetConstantBuffers(0, 3, cbsCube);
    g_ImmediateContext->PSSetConstantBuffers(2, 1, &g_MaterialBuffer);
    ...
    XMMATRIX modelTrans1 = XMMatrixTranslation(-1.5f, 0.0f, 0.0f) * XMMatrixRotationY(currentTime * 0.3f);
    XMVECTOR pos1 = modelTrans1.r[3];
    float dist1 = XMVectorGetX(XMVector3Length(pos1 - eye));

    XMMATRIX modelTrans2 = XMMatrixTranslation(1.5f, 0.0f, -2.0f) * XMMatrixRotationY(currentTime * 0.7f);
    ...
    g_ImmediateContext->OMSetDepthStencilState(g_DepthStateNoWrite, 0);

    for (const auto& obj : transparentObjects) {
        UpdateModelBuffer(obj.model);
        UpdateMaterialBuffer(obj.color.x, obj.color.y, obj.color.z, obj.color.w);
        g_ImmediateContext->DrawIndexed(36, 0, 0);
    }
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int nCmdShow) {
    ...
    HWND hMainWindow = CreateWindowEx(0, wc.lpszClassName, L"Lab5 - Depth & Transparency", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, WINDOW_WIDTH, WINDOW_HEIGHT, nullptr, nullptr, hInstance, nullptr);
    if (!hMainWindow) {
    ...
    if (FAILED(CreateExtraResources())) {
        MessageBox(nullptr, L"Extra resources creation failed", L"Error", MB_ICONERROR);
        DestroyD3DResources();
        return -1;
    }
